...
from fastapi import WebSocket, WebSocketDisconnect, BackgroundTasks, HTTPException, Depends, Header, UploadFile, File, Request
import httpx
...
async def lifespan(app: FastAPI):
    ...
    logger.info("🚀 Persistent Dev Proxy Client initialized")
    yield
...
plan_state_path = root_dir / "data" / "archivist_plan.json"
events_log_path = root_dir / "data" / "archivist_events.jsonl"

def _load_pending_plan():
    try:
        kb = get_icgl().kb
        return kb.get_metadata("archivist_pending_plan")
    except Exception as e:
        logger.warning(f"Failed to load pending plan from KB: {e}")
    return None

def _save_pending_plan(plan_data):
    try:
        kb = get_icgl().kb
        if plan_data is None:
            kb.delete_metadata("archivist_pending_plan")
        else:
            kb.save_metadata("archivist_pending_plan", plan_data)
    except Exception as e:
        logger.warning(f"Failed to persist pending plan to KB: {e}")

def _load_archivist_events():
    try:
        kb = get_icgl().kb
        logs = kb.get_recent_agent_logs(agent_id="Sovereign_Archivist", limit=50)
        # Reverse to maintain original chronological order or handle in UI
        archivist_agent.last_consultation_logs = sorted(logs, key=lambda x: x["timestamp"])
    except Exception as e:
        logger.warning(f"Failed to load archivist events from KB: {e}")

def _record_archivist_log(event: str, payload: Dict[str, Any]):
    """Append a short log entry to the archivist transparency buffer and KB."""
    try:
        kb = get_icgl().kb
        policy_code = payload.get("policy_code", "ARCHIVE")
        kb.save_agent_log(
            agent_id="Sovereign_Archivist",
            event_type=event,
            policy_code=policy_code,
            payload=payload
        )
        # Also update in-memory buffer for immediate UI response
        entry = {
            "agent_id": "Sovereign_Archivist",
            "event_type": event,
            "policy_code": policy_code,
        ...
        archivist_agent.last_consultation_logs.append(entry)
        archivist_agent.last_consultation_logs = archivist_agent.last_consultation_logs[-50:]
    except Exception as e:
        logger.warning(f"Failed to record archivist log in KB: {e}")
...
async def trigger_archivist_audit():
    ...
    try:
        ...
        _record_archivist_log("AUDIT_TRIGGERED", {"status": final_report.get("status") if isinstance(final_report, dict) else "OK"})
        return final_report
    except Exception as e:
...
# --- Plan State Store ---
pending_improvement_plan = _load_pending_plan()
...
async def plan_improvements():
    ...
    # Normalize status for UI workflow
    plan["status"] = plan.get("status", "PLAN_READY")
    
    # Store in memory for approval
    pending_improvement_plan = plan
    _save_pending_plan(pending_improvement_plan)
    _record_archivist_log("PLAN_IMPROVEMENTS", {"status": plan.get("status"), "missing": plan.get("consultant_plan", {}).get("required_policies")})
    
    return {
        "status": "PLAN_GENERATED",
        "summary": "Review pending plan via GET /archivist/plan",
        "plan": pending_improvement_plan
    }
...
async def execute_plan_action(payload: Dict[str, str]):
    ...
    if action == "REJECT":
        ...
        _save_pending_plan(None)
        _record_archivist_log("PLAN_REJECTED", {"action": action})
        return {"status": "PLAN_REJECTED", "message": "Plan discarded."}
        
    if action == "APPROVE":
        try:
            # Execute Drafting Phase
            drafts = await archivist_agent.create_drafts_from_plan(pending_improvement_plan, consultant_agent=external_consultant)
            
            pending_improvement_plan["status"] = "DRAFTS_READY"
            pending_improvement_plan["drafts"] = drafts
            _save_pending_plan(pending_improvement_plan)
            _record_archivist_log("PLAN_APPROVED", {"action": action, "drafts": drafts})
...
async def ratify_drafts():
    ...
    try:
        ...
        pending_improvement_plan = None
        _save_pending_plan(None)
        _record_archivist_log("RATIFY", {"ratified": ratified})
        ...
        logger.error(f"Ratify failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/archivist/dedup")
async def run_archivist_dedup(similarity_threshold: float = 0.8):
    """
    Run duplicate detection/merge flow with consultant guidance.
    """
    result = await archivist_agent.deduplicate_policies(external_consultant, similarity_threshold=similarity_threshold)
    return result

@app.post("/archivist/policy/archive")
async def archive_policy(filename: str):
    try:
        path = archivist_agent.archive_policy(filename)
        _record_archivist_log("POLICY_ARCHIVED", {"filename": filename})
        return {"status": "archived", "path": path}
    except Exception as e:
        logger.error(f"Archive policy failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))

@app.delete("/archivist/policy")
async def delete_policy(filename: str, is_draft: bool = False):
    try:
        path = archivist_agent.delete_policy(filename, is_draft=is_draft)
        _record_archivist_log("POLICY_DELETED", {"filename": filename, "is_draft": is_draft})
...
async def get_archivist_content(filename: str, is_draft: bool = False, doc_type: str = "policy"):
    """
    Fetch raw markdown content of a policy, draft, ADR, or procedure.
    doc_type: policy, draft, adr, procedure
    """
    try:
        # Backward compatibility for is_draft param
        if is_draft and doc_type == "policy":
            doc_type = "draft"
            
        if doc_type == "draft":
            base_dir = archivist_agent.drafts_dir
        elif doc_type == "adr":
            base_dir = archivist_agent.adrs_dir
        elif doc_type == "procedure":
            base_dir = archivist_agent.procedures_dir
        elif doc_type == "archived":
            base_dir = archivist_agent.archived_dir
        else:
            base_dir = archivist_agent.policies_dir
            
        file_path = base_dir / filename
        
        if not file_path.exists():
            raise HTTPException(status_code=404, detail=f"Document not found: {filename} in {doc_type}")
...
@app.post("/archivist/notes")
async def save_archivist_notes(payload: Dict[str, Any]):
    """
    Store sovereign owner's notes alongside a draft or policy.
    """
    filename = payload.get("filename")
    notes = payload.get("notes", "")
    is_draft = payload.get("is_draft", True)
    
    if not filename or not notes.strip():
        raise HTTPException(status_code=400, detail="filename and notes are required")
    ...
    try:
        notes_file = target_file.parent / f"{target_file.name}.owner-notes.md"
        notes_file.parent.mkdir(parents=True, exist_ok=True)
        stamped = f"## Note @ {datetime.utcnow().isoformat()} UTC\n\n{notes}\n\n---\n"
        existing = notes_file.read_text(encoding="utf-8") if notes_file.exists() else ""
        notes_file.write_text(existing + stamped, encoding="utf-8")

        # Apply notes via consultant to produce updated draft
        apply_result = await archivist_agent.apply_owner_notes(filename, notes, is_draft=is_draft, consultant_agent=external_consultant)

        _record_archivist_log("OWNER_NOTES", {"filename": filename, "applied": True, "notes": notes})
        return {"status": "saved", "file": str(notes_file), "updated": apply_result}
    except Exception as e:
        logger.error(f"Failed to save notes: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/archivist/upload")
async def upload_archivist_file(file: UploadFile = File(...), is_draft: bool = True):
    """
    Allow sovereign owner to upload supporting documents into drafts/policies.
    """
    try:
        suffix = Path(file.filename).suffix.lower()
        if suffix not in [".md", ".txt"]:
            raise HTTPException(status_code=400, detail="Only .md or .txt files are accepted")
        target_dir = archivist_agent.drafts_dir if is_draft else archivist_agent.policies_dir
        target_dir.mkdir(parents=True, exist_ok=True)
        target_path = target_dir / Path(file.filename).name
        content = await file.read()
        target_path.write_bytes(content)
        _record_archivist_log("OWNER_UPLOAD", {"filename": target_path.name})
        return {"status": "uploaded", "filename": target_path.name, "path": str(target_path)}
    except HTTPException:
        raise
    except Exception as e:
...
@app.get("/strategic/signals")
async def get_strategic_signals():
    """
    Placeholder endpoint to satisfy dashboard requests.
    """
    return {"signals": []}
...
async def list_policies():
    ...
    policies_list = []
    titles = {}
    for f in archivist_agent.policies_dir.glob("*.md"):
    ...
    return {"policies": sorted(policies_list, key=lambda x: x["name"]), "titles": titles}

@app.get("/policies/{name}")
async def get_policy_details(name: str):
    # Support both raw name and .md suffix
    filename = name if name.endswith(".md") else f"{name}.md"
    path = archivist_agent.policies_dir / filename
    if not path.exists():
        raise HTTPException(status_code=404, detail="Policy not found")
    
    content = path.read_text(encoding="utf-8")
    # Simple extraction for PolicyEditor schema
    return {
        "name": name,
        "description": content.split("\n")[0].lstrip("#").strip() if content else "",
        "type": "standard",
...
async def list_drafts():
    ...
    files = []
    titles = {}
    for f in archivist_agent.drafts_dir.glob("*.md"):
        if ".owner-notes" in f.name:
            continue
        files.append(f.name)
        titles[f.name] = _extract_title(f)

    return {"drafts": sorted(files), "titles": titles}

@app.get("/archivist/adrs")
async def list_adrs():
    if not archivist_agent.adrs_dir.exists():
        return {"adrs": [], "titles": {}}
    ...
    files = []
    titles = {}
    for f in archivist_agent.adrs_dir.glob("*.md"):
        files.append(f.name)
        titles[f.name] = _extract_title(f)

    return {"adrs": sorted(files), "titles": titles}

@app.get("/archivist/procedures")
async def list_procedures():
    if not archivist_agent.procedures_dir.exists():
        return {"procedures": [], "titles": {}}
    ...
    files = []
    titles = {}
    for f in archivist_agent.procedures_dir.glob("*.md"):
        files.append(f.name)
        titles[f.name] = _extract_title(f)

    return {"procedures": sorted(files), "titles": titles}

@app.get("/archivist/archived")
async def list_archived_policies():
    if not archivist_agent.archived_dir.exists():
    ...
    files = []
    titles = {}
    for f in archivist_agent.archived_dir.glob("*.md"):
        files.append(f.name)
        titles[f.name] = _extract_title(f)

    return {"archived": sorted(files), "titles": titles}
        
    return {"error": "Invalid action."}

# Load on startup
proposals_store = _load_proposals()
_load_archivist_events()
...
@app.delete("/proposals/{id_or_index}")
async def delete_proposal(id_or_index: str):
    """
    Delete a proposal by id or index.
    """
    # Try by id
    global proposals_store
    found = None
    for i, p in enumerate(proposals_store):
        if str(p.get("id")) == id_or_index:
            found = i
            break
    if found is None:
        # fallback: index if numeric
        if id_or_index.isdigit():
            idx = int(id_or_index)
            if 0 <= idx < len(proposals_store):
                found = idx
    if found is None:
        raise HTTPException(status_code=404, detail="Proposal not found")

    # Remove and persist
    removed = proposals_store.pop(found)
    try:
        kb = get_icgl().kb
        kb.delete_proposal(removed["id"]) if hasattr(kb, "delete_proposal") else None
    except Exception as e:
        logger.warning(f"Failed to delete proposal from KB: {e}")
    return {"status": "deleted", "proposal": removed}
...
def _get_health_summary():
    try:
        icgl = get_icgl()
        # Calculate a realistic integrity score
        integrity = 100
        status = "normal"
        ...
        # Count active agents
        active_agents = 3 # Fixed baseline
        if hasattr(icgl, "registry") and hasattr(registry, "agents"):
            active_agents = len(registry.agents)
            
        # Count active operations
        active_proposals = len(proposals_store)
        active_plans = 1 if pending_improvement_plan else 0
        active_ops = active_proposals + active_plans
        
        # If operations are overloaded
        if active_ops > 10:
            status = "overloaded"
            integrity -= 5
        ...
        return {"integrity_score": 0, "status": "critical", "active_agents": 0, "active_operations": 0}

@app.get("/health/system")
async def get_system_health(_: bool = Depends(_require_api_key)):
    return _get_health_summary()

# Alias without auth for frontend compatibility
...
async def get_channels_status(_: bool = Depends(_require_api_key)):
    ...
    try:
        ...
        # Agents Channel
        agent_cnt = len(registry.agents) if hasattr(registry, "agents") else 0
        channels.append({
            "id": "ch_core",
...
# --- Dashboard Mounting & Dev Proxy ---
# Switch to new React Build
ui_path = Path(__file__).parent.parent / "web" / "dist"

# Proving to be monolithic: even in dev, we prefer one port.
...
async def dashboard_proxy(path: str, request: Request):
    if VITE_DEV_PROXY:
        ...
        if proxy_client:
            try:
                # Proxy HTML and API-like calls to Vite dev server
                url = f"http://localhost:5173/dashboard/{path}"
...
async def run_analysis_task(adr: ADR, human_id: str) -> None:
    ...
    try:
        ...
        policy_report = enforcer.check_adr_compliance(adr)
        ...
        matches = await memory.search(query, limit=4)
        semantic = []
        for m in matches:
        ...
        alerts = await sentinel.scan_adr_detailed_async(adr, kb)
        ...
        synthesis = await registry.run_and_synthesize(problem, kb)
        ...
        if synthesis.overall_confidence < 0.7:
            ...
            llm_provider = registry.get_llm_provider() if hasattr(registry, "get_llm_provider") else None
            ...
            mediation_result = await mediator.analyze(problem_mediation, kb)
...
async def sign_decision(adr_id: str, req: SignRequest):
    try:
        ...
        # Record Decision
        decision = hdal.sign_decision(adr_id, req.action, req.rationale, req.human_id)
        ...
        if req.action == "APPROVE" and getattr(icgl, "engineer", None) and auto_write_enabled:
             ...
             if all_changes:
                 for change in all_changes:
                      engineer.write_file(change.path, change.content)
                 engineer.commit_decision(adr, decision)
...
async def list_kb(type: str) -> Any:
    try:
        icgl = get_icgl()
        if type == "adrs": return sorted(list(kb.adrs.values()), key=lambda x: x.created_at, reverse=True)
        if type == "policies": return list(kb.policies.values())
...
async def get_system_logs(limit: int = 50):
    ...
    for d in dashboard.decision_log[-limit:]:
        logs.append({
            "timestamp": datetime.utcnow().isoformat(), # Ideally use d['timestamp']
            "category": "DECISION",
            "source": "SovereignDesk",
            "message": f"Decision: {d.get('decision', 'N/A')} for proposal {d.get('proposal_idx', 'N/A')}",
            "details": d.get("rationale", "")
        })
        
    # 2. Proposal Actions
    for p in proposals_store[-limit:]:
        if " [🛡️ Safe Execution Verified]" in p.get("executive_brief", ""):
            logs.append({
                "timestamp": datetime.utcnow().isoformat(),
                "category": "EXECUTION",
                "source": "Enforcer",
                "message": f"Verified execution of proposal: {p.get('proposal', 'N/A')}",
                "details": p.get("details", "")
            })
            
    # Sort and trim
    return {"logs": sorted(logs, key=lambda x: x["timestamp"], reverse=True)[:limit]}

@app.get("/observability/trace/{trace_id}/graph")
async def get_trace_graph(trace_id: str):
    """Get trace visualization graph"""
    try:
        # Load events for this trace (mock or from file)
        # For now, searching in data/logs/archivist_events.jsonl for simple demo
        events = []
        if events_log_path.exists():
            with open(events_log_path, "r", encoding="utf-8") as f:
                for line in f:
                    entry = py_json.loads(line)
                    # Use policy_code or some other logic to simulate trace_id
                    if trace_id == "latest" or entry.get("policy_code") == trace_id:
                        events.append({
                            "id": f"evt_{len(events)}",
                            "type": "llm_called" if "llm" in entry.get("prompt", "").lower() else "agent_analysis",
                            "actor": "Archivist",
                            "timestamp": entry.get("timestamp", datetime.utcnow().isoformat()),
                            "status": "success",
                            "data": {"description_ar": entry.get("prompt")},
                            "tags": {"policy": entry.get("policy_code")}
                        })
        
        if trace_id == "latest" and not events:
             # Sample data if empty
             events = [{
                "id": "evt_root", "type": "agent_analysis", "actor": "Sovereign", 
                "timestamp": datetime.utcnow().isoformat(), "status": "success",
                "data": {"description_ar": "نظام الحوكمة نشط وجاهز للعمل"}, "tags": {}
             }]

        # Return structured data for TraceVisualization
        return {
            "trace_id": trace_id,
            "nodes": events,
            "edges": [],
...
async def list_recent_traces():
    """List recent trace summaries for the UI"""
    return {
        "traces": [
            {"id": "latest", "path": "/archivist/plan", "status": "success", "duration": "1.2s", "timestamp": "الآن"},
            {"id": "G-SEC", "path": "/security/audit", "status": "success", "duration": "0.8s", "timestamp": "منذ دقيقتين"},
            {"id": "D-PLAN", "path": "/governance/draft", "status": "error", "duration": "4.5s", "timestamp": "منذ ساعة"}
        ]
    }
...
async def get_archive_status():
    ...
    try:
        ...
        ledger = get_ledger()
        kb = get_icgl().kb
        status = await archivist_agent.audit_kb(kb)
        # Expose current drafts/plan to UI
        status["pending_plan"] = pending_improvement_plan or {}
        status["drafts_count"] = len(list(archivist_agent.drafts_dir.glob("*.md"))) if archivist_agent.drafts_dir.exists() else 0
        _record_archivist_log("ARCHIVE_STATUS", {"status": status.get("status"), "drafts": status.get("drafts_count")})
        return status
    except Exception as e:
