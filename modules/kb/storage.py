...
class StorageBackend:
    ...
    def save_roadmap_item(self, item: "RoadmapItem") -> None:
        ...
        with self._get_connection() as conn:
            conn.execute(
                ...
                (id, cycle, title, status, goals, governed_by_adr,
    ...
    def load_all_roadmap_items(self) -> List["RoadmapItem"]:
        ...
        from .schemas import (
            RoadmapItem,  # Local import to avoid circular dependency if any
        )
    ...
    def load_all_adrs(self) -> Dict[ID, ADR]:
        ...
        with self._get_connection() as conn:
            for row in conn.execute("SELECT * FROM adrs"):
                adr = self._row_to_adr(row)
                adrs[adr.id] = adr
        return adrs

    def load_adr(self, adr_id: ID) -> Optional[ADR]:
        """Loads a single ADR by ID."""
        with self._get_connection() as conn:
            row = conn.execute("SELECT * FROM adrs WHERE id = ?", (adr_id,)).fetchone()
            if row:
                return self._row_to_adr(row)
        return None

    def _row_to_adr(self, row: sqlite3.Row) -> ADR:
        """Converts a database row to an ADR object."""
        return ADR(
            id=row["id"],
            title=row["title"],
            status=row["status"],
