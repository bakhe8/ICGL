...
class DocsSnapshotLoader:
    ...
    def load_snapshot(self, file_ext: str = "*.md") -> DocumentSnapshot:
        ...
        all_paths = list(self.docs_path.rglob(file_ext))
        file_paths = []
        ...
        for p in all_paths:
            ...
            if any(
                ...
                for part in p.relative_to(self.docs_path).parts
            ):
                continue
            file_paths.append(p)

        logger.info(f"Filtered to {len(file_paths)} relevant documentation files")
        ...
        logger.info(
            f"✅ Snapshot loaded: {len(files)} files, "
    ...
    def get_file_tree_summary(self, snapshot: DocumentSnapshot) -> str:
        ...
        lines.append("")
        lines.append(
            f"Total: {snapshot.total_files} files, "
    ...
    def get_content_summary(
        self, snapshot: DocumentSnapshot, max_chars: int = 200
    ) -> str:
        """
