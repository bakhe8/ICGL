...
class PolicyAgent(Agent):
    ...
    async def _analyze(self, problem: Problem, kb) -> AgentResult:
        ...
        try:
            ...
            purpose_directive = (
                "FAILED TO LOAD PURPOSE DIRECTIVE - FALLBACK TO NARRATIVE AUDIT."
            )
        ...
        is_compliant = raw_json.get("purpose_compliance", True) and (
            purpose_score >= 50
        )
        ...
        if not is_compliant:
            concerns.append(
                f"VIOLATION: Decision Quality too low ({purpose_score}/100) per SED-01 Hard Realism."
            )

        # Logic Kernel metadata
        metadata = {
            "purpose_score": purpose_score,
            "compliance_status": "PASS" if is_compliant else "FAIL",
            "narrative_drift_check": "FAILED" if not is_compliant else "CLEAN",
        }
