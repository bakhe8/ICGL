...
from modules.llm.prompts import JSONParser
from .base import Agent, AgentResult, AgentRole, Problem
from .base import FileChange as AgentFileChange
...
class BuilderAgent(Agent):
    def __init__(self, llm_provider=None):
        super().__init__(
            agent_id="agent-builder", role=AgentRole.BUILDER, llm_provider=llm_provider
        )
    ...
    def _learn_patterns(self, target_file_path: str) -> dict:
        ...
        try:
            ...
            pattern_hints["common_imports"] = list(
                set(pattern_hints["common_imports"])
            )[:10]
...
"""
        ...
        **CONTEXT**: {problem.context}
        ...
        for attempt in range(max_attempts):
            ...
            file_changes_list = (
                parsed.file_changes if isinstance(parsed.file_changes, list) else []
            )
        ...
        for fc in file_changes_list:
            action = (fc.get("action", "CREATE") or "CREATE").upper()
            # Map action semantics to FileChange.mode (w=write/overwrite, a=append)
            mode = "w"
            if action == "APPEND":
                mode = "a"
            ...
            file_changes_objs.append(
                AgentFileChange(
                    path=fc.get("path", ""), content=fc.get("content", ""), mode=mode
                )
            )
