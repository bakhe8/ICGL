...
class RelayLogEntry:
    ...
    def to_executive_format(self) -> str:
        ...
        priority_indicator = "🔴" if self.priority == "high" else "🟡" if self.priority == "medium" else "🟢"
...
class SecretaryAgent(Agent):
    ...
    def __init__(self, llm_provider: Optional[Any] = None):
        ...
        from shared.python.llm.client import LLMClient
    ...
    async def _analyze(self, problem: Problem, kb) -> AgentResult:
        ...
        from shared.python.llm.prompts import SECRETARY_SYSTEM_PROMPT
        ...
        if not clarity_needed and technical_intent:
            try:
                ...
                print(f"   📡 [Secretary] Handoff to Architect: {technical_intent[:50]}...")
        ...
        return AgentResult(
            ...
            ambiguity_level="High" if ambiguity > 7 else "Medium" if ambiguity > 3 else "Low",
    ...
    async def _ask_llm_json(self, prompt: str, system_prompt: str) -> Dict[str, Any]:
        ...
        from shared.python.llm.client import LLMConfig

        config = LLMConfig(model="gpt-4-turbo-preview", temperature=0.3, timeout=30.0, json_mode=True)
...
"""
        ...
        try:
            ...
            return f"Decision required: {problem.title}. Review and approval needed for governance compliance."

    def _analyze_coordination_needs(self, problem: Problem, stakeholders: List[str]) -> str:
        """Analyze what coordination is needed across teams."""
        needs = []

        if len(stakeholders) > 2:
            needs.append("⚠️ Multi-team coordination required - suggest alignment meeting")
