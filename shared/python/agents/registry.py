...
from shared.python.core.llm import OpenAIProvider
...
class AgentRegistry:
    ...
    def get_agent(self, role: Any) -> Optional[Agent]:
        """
        Gets an agent by role or string ID/Identifier.
        Handles 'architect' (enum val) or 'agent-architect' (agent_id).
        """
        ...
        try:
            # 1. Try direct enum conversion (e.g. "architect")
            enum_key = AgentRole(role)
            if enum_key in self._agents:
                return self._agents[enum_key]
        except ValueError:
            pass

        # 2. Fallback: Search by agent_id or role value in values
        for agent in self._agents.values():
            if getattr(agent, "agent_id", "") == role or agent.role.value == role:
                return agent
    ...
    async def run_all(self, problem: Problem, kb) -> List[AgentResult]:
        ...
        """
        tasks = [agent.analyze(problem, kb) for role, agent in self._agents.items() if role != AgentRole.MEDIATOR]
        return await asyncio.gather(*tasks)

    async def run_single_agent(self, agent_identifier: str, problem: Problem, kb) -> Optional[AgentResult]:
        """
        ...
        try:
            ...
            for agent in self._agents.values():
                if agent.agent_id == agent_identifier or agent.role.value == agent_identifier:
                    target_agent = agent
                    break
    ...
    async def run_and_synthesize(
        self,
        problem: Problem,
        kb,
        allowed_agents: Optional[List[str]] = None,
        precomputed_results: Optional[List[AgentResult]] = None,
    ) -> SynthesizedResult:
        """Alias for run_and_synthesize_dynamic for compatibility."""
        return await self.run_and_synthesize_dynamic(problem, kb, allowed_agents, precomputed_results)
    ...
    async def run_and_synthesize_dynamic(
        ...
        if current_depth >= max_depth:
            print(f"[Registry] ⚠️ Max consultation depth ({max_depth}) reached. Terminating recursion.")
            return self._synthesize(precomputed_results or [])

        # Check Token Budget (Phase 12 Refactor)
        from shared.python.governance.budget import TokenBudget
        ...
        if not budget.check_usage(current_tokens):
            ...
            print(f"[Registry] ⚠️ Token Budget {status.state} ({status.used}/{status.limit}). Terminating cycle.")
            return self._synthesize(precomputed_results or [])
    ...
    def _synthesize(self, results: List[AgentResult]) -> SynthesizedResult:
        ...
        for result in results:
            for rec in result.recommendations:
                ...
                count = all_recommendations[rec_lower].get("count", 0)
                all_recommendations[rec_lower]["count"] = int(count or 0) + 1

        # Consensus = recommended by 2+ agents
        consensus: List[str] = [str(r["text"]) for r in all_recommendations.values() if int(r.get("count", 0)) >= 2]

        # If no consensus, take top recommendations
        if not consensus:
            consensus = [str(r["text"]) for r in list(all_recommendations.values())[:3]]
