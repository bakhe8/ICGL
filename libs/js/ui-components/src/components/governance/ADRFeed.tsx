...
export type ADRRecord = {
  id: string;
  title: string;
  status: "DRAFT" | "ACCEPTED" | "REJECTED" | string;
  context: string;
  created_at: string;
};

export interface ADRFeedProps {
  adrs: ADRRecord[];
            ...
            <div key={adr.id} className="p-3 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 transition-colors group">
              <div className="flex justify-between items-start mb-1">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[10px] text-white/30 font-mono">{new Date(adr.created_at).toLocaleDateString()}</span>
                  {onDelete && (
                    <button
              ...
              <div className="text-xs text-white/50 line-clamp-2">{adr.context}</div>
            </div>
          ))
        )}
      </div>
