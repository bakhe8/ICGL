    ...
    <div className="flex flex-col h-full bg-slate-50/50 rounded-2xl overflow-hidden border border-slate-100">
      <div className="p-4 border-b border-white bg-white/50 backdrop-blur-sm flex justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-slate-800 text-sm italic">{title}</span>
        </div>
      </div>
          ...
          <div key={i} className={`flex gap-3 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 shadow-sm ${
                msg.role === "user" ? "bg-indigo-600 text-white" : "bg-white text-slate-400 border border-slate-100"
              }`}
            >
              {msg.role === "user" ? <User size={14} /> : <Bot size={14} />}
            </div>
            <div
              className={`max-w-[80%] px-4 py-2.5 rounded-2xl text-sm shadow-sm border ${
                msg.role === "user" ? "bg-indigo-600 text-white border-indigo-700 rounded-tr-sm" : "bg-white text-slate-700 border-slate-100 rounded-tl-sm"
              }`}
            >
