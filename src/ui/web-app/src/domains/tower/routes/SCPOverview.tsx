...
const SCPOverview = () => {
    ...
    const loadStats = useCallback(async () => {
        try {
            const [obsData, chanData] = await Promise.all([
                fetchObservabilityStats(),
                fetchChannelStats()
            ]);

            setStats({
                observability: obsData,
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        {stats.observability.active_alert_level && <StatRow label="Alert Level" value={stats.observability.active_alert_level} />}
                    </div>
                </div>
