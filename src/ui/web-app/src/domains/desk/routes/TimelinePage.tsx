...
import { listDecisions, listGovernanceTimeline, listProposals } from '@web-src/domains/desk/api';
import type { GovernanceEvent } from '@web-src/domains/desk/types';
...
export default function TimelinePage() {
    ...
    const allEvents = [...liveTimeline, ...historicalEvents].sort((a, b) => {
        const timeA = new Date((a as any).timestamp || (a as any).time || 0).getTime();
        const timeB = new Date((b as any).timestamp || (b as any).time || 0).getTime();
        return timeB - timeA;
    });
                                    ...
                                    <div className="p-4 rounded-2xl border border-slate-200 bg-white/70 hover:border-brand-base/30 hover:bg-white transition-all">
                                        <div className="flex items-center justify-between mb-1">
                                            ...
                                            <span className="text-[11px] text-slate-400 font-mono">
                                                {new Date((event as any).timestamp || (event as any).time).toLocaleString()}
                                            </span>
                                        </div>
                ...
                <aside className="space-y-6">
                    ...
                    <div className="glass rounded-3xl p-6 space-y-4">
                        ...
                        <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                            {liveTimeline.map((e: any, i: number) => (
