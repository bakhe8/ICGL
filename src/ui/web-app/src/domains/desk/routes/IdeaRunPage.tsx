...
const IdeaRunPage = () => {
  ...
  const startPolling = (id: string) => {
    ...
    pollRef.current = window.setInterval(async () => {
      try {
        const data: AnalysisResponse = await fetchAnalysis(id);
        setAnalysis(data);
  ...
  const submitIdea = async () => {
    ...
    setAdrId(null);
    try {
      const data = await runIdea({ idea });
      setAdrId(data.adr_id);
      if (data.adr_id) {
        startPolling(data.adr_id);
      }
    } catch (err: any) {
  ...
  const approveChanges = async () => {
    ...
    try {
      const data = await approveChangesQuery({
        adr_id: adrId,
        rationale: rationale
      });
  ...
  const triggerRebuild = async () => {
    ...
    try {
      const data = await rebuildSystem();
      if (data.status === 'ok') {
  ...
  const secretaryResult = agentResults.find(r => r.agent_id === 'secretary' || r.agent_id === 'agent-secretary');
                  ...
                  {policyReport && (
                    <div className="bg-white border border-slate-200 p-6 rounded-xl space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`text-[10px] px-2 py-0.5 rounded-full font-black ${policyReport.status === 'FAIL'
