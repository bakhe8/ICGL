    ...
    return (
        ...
        ${isUser ? 'bg-indigo-500' : 'bg-emerald-600'}`}>
            ...
            <div className={`flex flex-col gap-2 max-w-[80%] ${isUser ? 'items-end' : 'items-start'}`}>
                <div className={`px-4 py-3 rounded-2xl glass-panel border-0
           ${isUser
                ...
                {!isUser && message.blocks && message.blocks.length > 0 && (
                    <div className="flex flex-col gap-2 w-full mt-1">
                        {(message.blocks as ChatBlock[]).map((block, i) => (
...
const BlockRenderer = ({ block, onAction }: { block: ChatBlock; onAction?: (a: string) => void }) => {
    ...
    return (
        <div className="bg-white/5 border border-white/10 rounded-lg p-3 text-sm">
            ...
            <pre className="whitespace-pre-wrap text-white/50 text-xs font-mono">
