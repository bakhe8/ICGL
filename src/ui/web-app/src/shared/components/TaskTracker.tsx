...
const TaskTracker: React.FC<TaskTrackerProps> = ({ objective, tasks }) => {
    return (
        <motion.div
                ...
                <AnimatePresence mode="popLayout">
                    {tasks.map((task, idx) => (
                        <motion.div
                            ...
                            className={`flex items-start gap-3 p-3 rounded-2xl border transition-all ${task.status === 'in-progress'
                        ...
                        >
                            ...
                            <span className={`text-xs font-medium leading-tight ${task.status === 'completed' ? 'text-white/40 line-through' : 'text-white/80'
                                }`}>
                                {task.text}
                            </span>
                        </motion.div>
            ...
            <div className="mt-6 pt-4 border-t border-white/5 flex items-center justify-between">
                <div className="flex gap-1">
                    {tasks.map((t, i) => (
                        <div
                            key={i}
                            className={`h-1 w-4 rounded-full ${t.status === 'completed' ? 'bg-emerald-500' :
                                    t.status === 'in-progress' ? 'bg-indigo-500' : 'bg-white/10'
                                }`}
                        />
                    ))}
                </div>
