...
export default function CockpitPage() {
  ...
  const { events, status } = useGovernanceStream(); // Real-time connection
  ...
  return (
    <div className="space-y-8 pt-6 pb-20 relative">
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        {/* Left Column: Decision Center (8 cols) */}
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-2 px-3 py-1 bg-slate-50 rounded-full border border-slate-200">
                ...
                <span className="text-[10px] font-black tracking-widest text-slate-600">
                  {status === 'CONNECTED' ? 'LIVE FEED' : 'OFFLINE'}
                </span>
              </div>
            </div>
            <SecretaryLogsWidget extraLogs={events} />
          </section>
          burial:
