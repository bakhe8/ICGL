...
from src.core.kb.schemas import AgentManifestEntry
...
class CapabilityChecker:
    ...
    def _load_manifest(self):
        """Load agent manifest from KB."""
        if not self.kb.agents:
            self._bootstrap_initial_agents()
        self.agents = self.kb.agents

    def _bootstrap_initial_agents(self):
        """Seeds the initial manifest if empty."""
        initial = {
            "architect": AgentManifestEntry(
                id="architect",
                file="architect.py",
                role="ARCHITECT",
                capabilities=["structural_analysis", "design_patterns", "coupling_analysis", "architecture_review"],
                status="active",
            ),
            "builder": AgentManifestEntry(
                ...
                file="builder.py",
                role="BUILDER",
                capabilities=["code_generation", "pattern_learning", "self_verification", "file_creation"],
                status="active",
            ),
            ...
            "policy": AgentManifestEntry(
                ...
                file="policy.py",
                role="POLICY",
                capabilities=["policy_compliance", "rule_enforcement", "violation_detection"],
                status="active",
            ),
            "sentinel": AgentManifestEntry(
                ...
                role="SENTINEL",
                capabilities=["risk_detection", "drift_monitoring", "real_time_analysis"],
                status="active",
            ),
            "documentation": AgentManifestEntry(
                ...
                role="DOCUMENTATION",
                capabilities=["documentation_generation", "content_analysis", "quality_assessment"],
                status="active",
            ),
            ...
            "testing": AgentManifestEntry(
                ...
                file="testing.py",
                role="TESTING",
                capabilities=["test_generation", "unit_testing", "coverage_analysis"],
                status="active",
            ),
            "verification": AgentManifestEntry(
                ...
                role="VERIFICATION",
                capabilities=["deep_verification", "security_audit", "quality_gate"],
                status="active",
            ),
            "mediator": AgentManifestEntry(
                ...
                file="mediator.py",
                role="MEDIATOR",
                capabilities=["conflict_resolution", "consensus_building", "active_intervention"],
                status="active",
            ),
            "capability_checker": AgentManifestEntry(
                ...
                capabilities=["gap_analysis", "redundancy_prevention", "ecosystem_governance"],
                status="active",
            ),
        }
        for entry in initial.values():
            self.kb.add_agent_entry(entry)
