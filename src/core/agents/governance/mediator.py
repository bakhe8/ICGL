...
class MediatorAgent(Agent):
    ...
    async def _analyze(self, problem: Problem, kb) -> AgentResult:
        ...
        _low_confidence_agents = [r["agent_id"] for r in other_results if r.get("confidence", 1.0) < 0.7]
        interpretations = [
            f"{r['agent_id']}: {r.get('understanding', {}).get('interpretation', 'N/A')}" for r in other_results
        ]
        ...
        mediation_prompt = (
            f"You are the ICGL Mediator Agent. Your role is to EXPOSE tensions between agents and PROVIDE high-efficacy resolutions.\n\n"
            ...
            "Your resolution MUST focus on efficacy: Why is this the best path forward for the system?\n"
            'OUTPUT JSON: {"tensions": [{"dimension": "...", "left_agent": "...", "right_agent": "...", "description": "...", "suggested_resolution": "...", "efficacy_narrative": "..."}], "analysis": "...", "recommendations": []}'
        )
        ...
        return AgentResult(
            ...
            confidence=0.5 if tensions else 1.0,  # Lower confidence if tensions exist to trigger review
            tensions=tensions,
            clarity_needed=len(tensions) > 0,
            clarity_question=f"Tensions detected: {len(tensions)} issues require human review." if tensions else None,
        )
