...
class ICGL:
    ...
    async def run_governance_cycle(self, adr: ADR, human_id: str) -> Optional[HumanDecision]:
        ...
        if policy_report.status == "FAIL":
            print(f"   ⛔ CRITICAL POLICY VIOLATION. Stopping.")
            return None
        elif policy_report.status == "ESCALATE":
            print(f"   ⚠️ Policy Violation Detected (ESCALATING to HDAL).")
        else:
        ...
        await self.memory.add_document(Document(
            id=f"adr-{adr.id}",
        ...
        if decision.action == "APPROVE" and getattr(self, 'engineer', None):
            print(f"[DEBUG] Engineer detected. checking results...")
            all_changes = []
