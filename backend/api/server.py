...
async def get_conflicts():
    ...
    from shared.python.governance_shared.governance.conflicts import conflict_registry
...
async def list_agents():
    ...
    try:
        ...
        try:
            from shared.python.agents_shared.agents.metadata import get_agent_metadata
        ...
        if (
            ...
            for res in result_data["synthesis"]["agent_results"]:
                if "file_changes" in res and res["file_changes"]:
                    from shared.python.kb_shared.kb.schemas import FileChange
...
async def get_trace_graph(trace_id: str):
    ...
    try:
        from shared.python.observability_shared.observability import get_ledger
        from shared.python.observability_shared.observability.graph import TraceGraphBuilder
...
async def scp_websocket(websocket: WebSocket):
    ...
    from shared.python.observability_shared.observability.broadcaster import get_broadcaster
...
async def get_pattern_alerts(limit: int = 10):
    ...
    try:
        from shared.python.observability_shared.observability import get_ledger
        from shared.python.observability_shared.observability.patterns import get_detector
...
async def get_ml_status():
    ...
    try:
        from shared.python.observability_shared.observability.ml_detector import get_ml_detector
