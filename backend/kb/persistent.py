...
class PersistentKnowledgeBase:
    ...
    def __init__(
        self, db_path: str = "data/kb.db", validate: bool = True, bootstrap: bool = True
    ):
        """
        Args:
        ...
        self.human_decisions: Dict[ID, HumanDecision] = (
            self._storage.load_all_human_decisions()
        )
    ...
    def remove_adr(self, adr_id: ID) -> bool:
        ...
        self.human_decisions = {
            key: val
            for key, val in self.human_decisions.items()
    ...
    def get_adr(self, adr_id: ID) -> Optional[ADR]:
        ...
        if not adr:
            # Fallback: check storage directly in case of distributed writes or cache issues
            adr = self._storage.load_adr(adr_id)
            if adr:
