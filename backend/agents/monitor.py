...
"""
        ...
        return AgentResult(
            ...
            concerns=[
                ...
                if resource_needs
                else "Resource impact expected to be minimal",
                f"{len(health.issues)} system issues detected"
                if health.issues
                else "No current system issues",
    ...
    def _get_system_health(self) -> HealthStatus:
        ...
        try:
            ...
            health.issues = self._check_thresholds(
    ...
    def _check_thresholds(self, cpu: float, memory: float, disk: float) -> List[str]:
        ...
        if cpu > self.alert_thresholds["cpu"]:
            issues.append(
                f"⚠️ CPU usage high: {cpu:.1f}% "
                f"(threshold: {self.alert_thresholds['cpu']}%)"
            )

        if memory > self.alert_thresholds["memory"]:
            issues.append(
                f"⚠️ Memory usage high: {memory:.1f}% "
                f"(threshold: {self.alert_thresholds['memory']}%)"
            )
...
"""
    ...
    def _estimate_resource_needs(self, problem: Problem) -> str:
        ...
        if any(
            kw in context_lower
            for kw in ["database", "migration", "index", "large dataset"]
        ):
        ...
        if any(
            kw in context_lower
            for kw in ["computation", "processing", "analysis", "ml"]
        ):
