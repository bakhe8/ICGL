...
class DocsRefactorPipeline:
    ...
    def __init__(self, docs_path: Path = None, staging_root: Path = None):
        """
    ...
    def load_snapshot(self) -> DocumentSnapshot:
        ...
        logger.info(
            f"✅ Snapshot loaded: {snapshot.total_files} files, "
    ...
    def stage_files(
        self, plan: Optional[RewritePlan] = None, session_id: Optional[str] = None
    ) -> StagingManifest:
        """
        ...
        if not is_valid:
            logger.error(f"Documentation analysis produced invalid plan: {errors}")
            raise ValueError(f"Invalid RewritePlan: {', '.join(errors)}")
