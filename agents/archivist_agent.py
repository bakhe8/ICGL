...
class ArchivistAgent(Agent):
    ...
    def detect_missing_policies(self) -> List[str]:
        ...
        logger.info(f"📋 Policy Scan: {len(referenced)} referenced, {len(missing)} missing")
        return missing
...
OUTPUT FORMAT:
# الموجز الاستراتيجي (Strategic Brief)
...
"""
    ...
    async def generate_policy_with_llm(self, policy_code: str, context: str = "", strategic_brief: str = "") -> str:
        ...
        logger.info(f"🤖 Generating content for {policy_code} using LLM...")
        
        if not self.llm:
            logger.warning("⚠️ LLM not available, using template-based generation")
...
LANGUAGE: You MUST write the entire document in ARABIC (اللغة العربية). Use professional, executive-level Arabic terms (e.g., 'نطاق العمل', 'المسؤوليات', 'الإنفاذ').
...
"""
        try:
            ...
            logger.info(f"✅ Generated policy {policy_code} via LLM consultation")
            return response.content
        except Exception as e:
            logger.error(f"❌ LLM consultation failed: {e}, falling back to template")
            return self.generate_policy_draft(policy_code)

    def detect_policy_contradictions(self, kb) -> List[Dict[str, str]]:
        """
        فحص التضارب بين السياسات والوثائق.
    ...
    async def check_document_coherence(self, kb) -> Dict[str, Any]:
        """
        التأكد من ترابط الوثائق وعدم تضاربها.
        ...
        if consultant_agent:
            try:
                ...
                if review.get("score", 0) < 50 or "[To be defined]" in str(review.get("critique", "")):
                    logger.info("🔄 Consultant feedback too generic, challenging for more detail...")
                    review = await consultant_agent.review_document_draft(
    ...
    async def create_missing_policy_files(self, missing_policies: List[str]) -> Dict[str, str]:
        ...
        for policy_code in missing_policies:
            ...
            try:
                ...
                logger.info(f"✅ Created policy draft: {policy_file}")
            except Exception as e:
                logger.error(f"❌ Failed to create {policy_code}: {e}")
    ...
    async def _analyze(self, problem: Problem, kb) -> AgentResult:
        ...
        if audit['missing_policy_files']:
            analysis += f"\n\n⚠️ DETECTED {len(audit['missing_policy_files'])} MISSING POLICY FILES: {', '.join(audit['missing_policy_files'])}"
    ...
    async def submit_report_to_ceo(self, kb, secretary_agent=None, consultant_agent=None, mediator_agent=None) -> Dict[str, Any]:
        ...
        if mediator_agent and consultant_agent:
            try:
                ...
                logger.info(f"🧠 Synthesizing Strategic Brief for plan...")
                logger.info(f"🚦 Mediator Traffic Control: {msg}")
                ...
                if mediation_result["approved"]:
                    ...
                    logger.info("✅ Mediator Approved Consultation")
                else:
                    logger.info(f"⛔ Mediator Denied Consultation: {mediation_result['reason']}")
    ...
    async def report_blocker(self, blocker_type: str, details: str, severity: str = "HIGH") -> Dict[str, str]:
        """
        رفع تقرير معوق للرئيس عبر المكتب.
        ...
        logger.critical(f"🚨 BLOCKER REPORTED: {blocker_type} - {details}")
    ...
    async def create_drafts_from_plan(self, plan_data: Dict[str, Any], consultant_agent=None) -> List[str]:
        ...
        logger.info(f"🧠 Strategic Brief Status: {len(strategic_brief) if strategic_brief else 'Missing'} chars")
        ...
        if not policy_codes:
            logger.info("⚠️ No specific policy codes identified, generating P-GEN-01 fallback.")
        ...
        for code in policy_codes:
            ...
            if draft_file.exists():
                ...
                logger.info(f"⏭️ Draft already exists for {code}, keeping current draft.")
                continue
            ...
            if code in existing_policies:
                try:
                    ...
                    logger.info(f"📄 Copied existing policy into drafts for review: {src_file} -> {draft_file}")
                except Exception as e:
            ...
            created_drafts.append(code)
            logger.info(f"📝 Created ROBUST DRAFT: {draft_file}")
            self.last_consultation_logs.append({
                "policy_code": code,
            ...
            try:
                ...
                if policy_file.exists():
                    ...
                    try:
                        ...
                        logger.info(f"📦 Archived existing policy before ratify: {archive_target}")
                ...
                logger.info(f"✅ RATIFIED Policy: {policy_file}")
            except Exception as e:
...
"""
                ...
                logger.info(f"🧹 Merged duplicate {duplicate_path} -> {primary_path}")
                self.last_consultation_logs.append({
...
"""
    ...
    def archive_policy(self, filename: str) -> str:
        ...
        logger.info(f"📦 Archived policy {source} -> {target}")
        return str(target)
    ...
    async def generate_improvement_plan(self, consultant_agent, kb=None) -> Dict[str, Any]:
        ...
        if self.policies_dir.exists():
            for f in self.policies_dir.glob("*.md"):
                # تخطَّ ملفات الملاحظات المرافقة
            ...
            try:
                ...
                if not getattr(consultant_agent, "has_intelligence", False):
                    ...
                    plan = {
                        ...
                        "consultant_plan": {
                            "approved": False,
                            "critique": "خطة هيوريستية: إنشاء سياسات أساسية ومسودات لمواضيع من المستندات.",
                            "required_policies": filtered_required,
                            "relations": [],
                            "structure_feedback": "Offline heuristic plan",
                            "proposed_actions": ["إنشاء المسودات المفقودة", "مراجعة التعارضات بعد الإنشاء"]
                        },
                        "status": "PLAN_READY",
                ...
                # تخلص من ملاحظات المالك إن وُجدت ضمن القائمة
                required = [c for c in required if ".owner-notes" not in str(c)]
                existing_codes = {Path(p).stem for p in inventory["policies"]}
                # Allow re-proposing processed codes if لا يوجد ملف فعلي؛ فقط تجنب الموجود على القرص
                ...
                if not filtered_required:
                    ...
                    if missing_from_audit:
                        ...
                        if missing_baseline:
                            ...
                            review["critique"] = "لا يوجد نقص ظاهر في الأكواد الحالية، لكن ينقصنا سياسات أساسية افتراضية (Incident/BC/AI/Vendor)."
                        else:
                            # take all existing policies and plan a QA pass
                            filtered_required = [c for c in existing_codes if c]
                            review["critique"] = "لا يوجد نقص ظاهر، سيتم إعداد مسودات مراجعة جودة للسياسات الحالية."
                            review["proposed_actions"] = review.get("proposed_actions") or [
                                "تجهيز نسخ مراجعة جودة لكل السياسات الحالية",
                                "مراجعة التكرار/التعارض بين السياسات",
                                "توحيد التنسيق والبنية"
                            ]
                ...
                if missing_from_audit:
                    review["critique"] = f"السياسات الناقصة حالياً: {', '.join(missing_from_audit)}"
                elif filtered_required:
                    review["critique"] = "سيتم إنشاء مسودات مراجعة جودة للسياسات الحالية."
                    review["proposed_actions"] = review.get("proposed_actions") or [
                        "تجهيز نسخ مراجعة جودة لكل السياسات الحالية",
                        "مراجعة التكرار/التعارض بين السياسات",
                        "توحيد التنسيق والبنية"
                    ...
                    review["critique"] = "لا يوجد نقص ظاهر في الأكواد، لكن يوصى بتدقيق الجودة وتوحيد البنية (تنظيف، توحيد أقسام، مراجعة التعارضات)."
                ...
                return {
                    ...
                    "consultant_plan": {
                        ...
                        "proposed_actions": ["إنشاء المسودات المفقودة"]
                    },
                    "status": "PLAN_READY",
                    "next_step": "User Approval"
                }
