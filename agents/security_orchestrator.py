...
class SecurityOrchestratorAgent(Agent):
    ...
    async def _analyze(self, problem: Problem, kb) -> AgentResult:
        ...
        guardian_res = await self.consult_peer(
            AgentRole.GUARDIAN, problem.title, problem.context, kb
        )
        sentinel_res = await self.consult_peer(
            AgentRole.SENTINEL, problem.title, problem.context, kb
        )
        chaos_res = await self.consult_peer(
            AgentRole.CHAOS, problem.title, problem.context, kb
        )
